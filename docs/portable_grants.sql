...
GRANT SELECT ON ALL SEQUENCES IN SCHEMA appraiser TO dbuser;
GRANT SELECT,INSERT,DELETE ON SCHEMA appraiser TO dbuser;
CREATE TABLE appraiser.tables( id serial NOT NULL, name character varying(200), alias varchar(200),pid int,tid int,filename varchar(200), geometrytype character varying(50), filetype character varying(50),  date_loaded timestamp without time zone, numtuples int default 0, type int default 0	)	WITH (	  OIDS=FALSE	);
